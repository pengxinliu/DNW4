...
class FourLayer_64F(nn.Module):
	...
	def forward(self, input1, input2):
		...
		for i in range(len(input2)):
			...
			B, C, h, w = support_set_sam.size() #5 64 21 21
			#support_set_sam = torch.mean(support_set_sam, dim=0, keepdim=True)
			#support_set_sam = support_set_sam.permute(1, 0, 2, 3)     # 64 5 21 21
...
class ImgtoClass_Metric(nn.Module):
	...
	def cosin_distance(self, input, k):
		w = []
		support_set_sam = input[k]
		...
		for i in range(5):
			if(i != k):
				ind[n] = i
				n = n + 1
		innerproduct_matrix1 = support_set_sam@input[ind[0]]
		topk_value1, topk_index1 = torch.topk(innerproduct_matrix1, self.feature_num, 1)
		innerproduct_matrix2 = support_set_sam@input[ind[1]]
		topk_value2, topk_index2 = torch.topk(innerproduct_matrix2, self.feature_num, 1)
		innerproduct_matrix3 = support_set_sam@input[ind[2]]
		topk_value3, topk_index3 = torch.topk(innerproduct_matrix3, self.feature_num, 1)
		innerproduct_matrix4 = support_set_sam@input[ind[3]]
		topk_value4, topk_index4 = torch.topk(innerproduct_matrix4, self.feature_num, 1)
		for i in range(len(topk_value1)):
			w.append(torch.div(topk_value1[i][self.feature_num-1]+topk_value2[i][self.feature_num-1]+topk_value3[i][self.feature_num-1]+topk_value4[i][self.feature_num-1], topk_value1[i][0]+topk_value2[i][0]+topk_value3[i][0]+topk_value4[i][0]))
		return w
	...
	def cal_cosinesimilarity(self, input1, input2):
		...
		for j in range(len(input2)):
			support_set = input2[j]
			support_set_sam = []
			for k in range(len(support_set)):
				w = self.cosin_distance(support_set, k)
				w = torch.Tensor(w).cuda()
				support_set_sam.append(torch.mul(support_set[k], w))
			support_set_sam = torch.cat(support_set_sam, 1)
			#support_set_sam = torch.transpose(support_set_sam, 0, 1)
			support_set_sam_norm = torch.norm(support_set_sam, 2, 0, True)
			support_set_sam = support_set_sam/support_set_sam_norm
			input3.append(support_set_sam)
		for i in range(B):
			query_sam = input1[i]
			...
			for j in range(len(input3)):
				...
				innerproduct_matrix = query_sam@support
				#matrix = torch.mul(innerproduct_matrix, weight[j])
				matrix = torch.transpose(innerproduct_matrix, 0, 1)

				# choose the top-k nearest neighbors
				topk_value, topk_index = torch.topk(matrix, self.neighbor_k, 1)
